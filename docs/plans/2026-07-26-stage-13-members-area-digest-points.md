...
4. **Digest audience and unsubscribe.** Recipients are `store_customers` docs carrying an email (members with an email, per the brief). The digest respects the marketing list's opt-out: any recipient whose `store_subscribers` doc (id = lower-cased email) has `unsubscribedAt` set is skipped, and every send carries the signed unsubscribe link, which writes to that same subscriber doc. A member who never joined the marketing list still gets the digest (spec 7.3 makes it a membership email), but one click of unsubscribe stops it for good because the route `set({merge:true})`s the subscriber doc into existence.
